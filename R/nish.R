...
#'
#'@examples
#'scale_colour_nish_blue <- nish:::create_discrete_scale(aes = "colour",
...
create_discrete_scale <- function(aes = c("colour", "fill", "linetype"),
  ...
  function(...) {
    ggplot2::discrete_scale(aesthetics = aes,
                            scale_name = "nish",
                            palette = pal, ...)
  }
}
...
# continous (gradient) colour and fill scales ----------
...
#'@param col A character string of gradient nuanse. Allowed values are "blue",
#'  "grey" and "pink".
...
#'
#'@examples
#'scale_fill_gradient_nish_blue <- nish:::create_gradient_scale(col = "blue",
#'aes = "fill")
#'
...
create_gradient_scale <- function(col = c("blue", "grey", "pink"),
  ...
  function(...) {
    if (aes == "colour") {
      ...
      ggplot2::scale_fill_gradient(low = low,
                                   high = high,
                                   na.value = na_value,
                                   aesthetics = aes,
                                   ...)
...
#'
#' @param ... Additional arguments are passed to
#'   \code{ggplot2::scale_colour_gradient}() or
...
#'@param aes A character aesthetic. Allowed values are "colour" and "fill".
#'
#'@return A function that returns a diverging colour gradient scale when called.
#'
#'@examples
#'scale_colour_gradient2_nish_pos_neg <- nish:::create_gradient2_scale(
#'type = "pn1", aes = "colour")
#'
...
create_gradient2_scale <- function(type = NULL,
  ...
  function(...) {
    if (aes == "colour") {
      ggplot2::scale_colour_gradient2(low = low,
                                      mid = mid,
                                      high = high,
                                      na.value = na_value,
                                      aesthetics = aes,
...
#'
#' @param ... Additional arguments are passed to
#'   \code{ggplot2::scale_colour_gradient2}() or
...
#'@param aes A character aesthetic. Allowed values are "colour" and "fill".
#'
#'@return A function that returns a gradientn scale when called. Defaults to use
#'  the following colours, \code{nish_colours[c(7, 5, 3:1)]}.
#'
#'@examples
#'scale_colour_gradientn_nish <- nish:::create_gradientn_scale(aes = "colour")
#'
...
#'
#' @param ... Additional arguments are passed to
#'   \code{ggplot2::scale_colour_gradientn}() or
...
#' nish themes
#'
#' nish themes with light blue ("#e4f2ff"), light pink ("#fcebe3") and white
#' backgrounds.
#' @name theme_nish
NULL

# base theme that applies to all theme_nish_* themes
